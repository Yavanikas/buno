...
export default function Home() {
  ...
  function handleDeleteExpense(expenseId: unknown) {
    ...
    if (!expenses.some((expense) => expense.id === expenseId)) {
      setFormMessage('Could not find that expense. Please try again.');
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-[#f8efe4] text-[#3a2118]">
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[8rem_minmax(0,1fr)] lg:gap-8 lg:px-8 lg:py-8">
        ...
        <div className="min-w-0">
          {activeTab === 'overview' ? (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <section className="flex min-w-0 flex-col gap-6 lg:col-span-2">
                <header className="rounded-[2rem] border border-[#ead9c8] bg-[#fffaf3]/85 p-5 shadow-[0_20px_60px_rgba(76,45,33,0.10)] sm:p-7">
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#9b7968]">Buno</p>
                  <h1 className="mt-2 max-w-3xl text-3xl font-bold tracking-tight text-[#3a2118] sm:text-4xl lg:text-5xl">
                    Minimalist Budget Overview
                  </h1>
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-[#735747] sm:text-base">
                    A calm student budget assistant that tracks your setup, reads your spending pace, and keeps guidance qualitative.
                  </p>
                </header>

                <div className="grid grid-cols-1 gap-6 md:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
                  <ActivityRhythmCard summary={monthlyPatternSummary} totalExpensesLogged={totalExpensesLogged} />
              ...
              <aside className="flex min-w-0 flex-col gap-6 rounded-[2rem] border border-[#f3dd9d] bg-[#fef9c3] p-5 shadow-[0_24px_70px_rgba(126,82,34,0.16)] sm:p-6 lg:col-span-1">
                <PatternInsight
                  riskLabel={budgetState.riskLabel}
              ...
              <section className="rounded-[1.75rem] border border-[#ead9c8] bg-[#fffaf3] p-5 shadow-[0_18px_48px_rgba(76,45,33,0.10)] sm:p-6 lg:col-span-3">
                <AddExpenseForm
                  amount={amount}
                  category={category}
                  date={date}
          ...
          {activeTab === 'patterns' ? (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <section className="flex min-w-0 flex-col gap-6 lg:col-span-2">
                <header className="rounded-[2rem] border border-[#ead9c8] bg-[#fffaf3]/85 p-5 shadow-[0_20px_60px_rgba(76,45,33,0.10)] sm:p-7">
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#9b7968]">Patterns</p>
                  <h1 className="mt-2 text-3xl font-bold tracking-tight text-[#3a2118] sm:text-4xl">Monthly behavior view</h1>
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-[#735747] sm:text-base">
                    A deterministic look at your current-month activity, with AI kept as a small supporting insight.
                  </p>
                </header>
                <MonthlyPatternsSection summary={monthlyPatternSummary} />
              </section>

              <aside className="rounded-[2rem] border border-[#f3dd9d] bg-[#fef9c3] p-5 shadow-[0_24px_70px_rgba(126,82,34,0.16)] sm:p-6 lg:col-span-1">
                <PatternInsight
                  riskLabel={budgetState.riskLabel}
                  recentExpenses={expenses}
                  daysRemaining={daysRemaining}
...
function ActivityRhythmCard({
  summary,
  totalExpensesLogged,
}: {
  summary: MonthlyPatternSummary;
  totalExpensesLogged: number;
}) {
  return (
    <section className="flex min-h-72 flex-col justify-between rounded-[1.75rem] border border-[#ead9c8] bg-[#fffaf3] p-5 shadow-[0_18px_48px_rgba(76,45,33,0.10)] sm:p-6">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#a48673]">Activity rhythm</p>
        <h2 className="mt-3 text-2xl font-bold tracking-tight text-[#3a2118]">Quick Stats</h2>
        <p className="mt-3 text-sm leading-6 text-[#7d6253]">{summary.currentMonthActivitySummary}</p>
      </div>

      <div className="mt-6 grid gap-3">
        <PatternStat label="Logged this session" value={String(totalExpensesLogged)} />
        <PatternStat label="Most frequent category" value={toTitleCase(summary.mostFrequentCategory)} />
        <PatternStat label="Current rhythm" value={summary.hasClusteredSpending ? 'Activity appears a little bursty' : 'Activity appears fairly spread out'} />
      </div>
    </section>
  );
}
...
function SetupSummaryCard({
  ...
  return (
    <section className="rounded-[1.75rem] border border-[#ead9c8] bg-[#fffaf3] p-5 shadow-[0_18px_48px_rgba(76,45,33,0.10)] sm:p-6">
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="min-w-0 rounded-3xl border border-[#ead9c8] bg-white/70 p-4">
          ...
          <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="break-words text-lg font-bold text-[#3a2118]">
              ₹{Math.round(monthlyBudget).toLocaleString('en-IN')}
            </p>
            <button
              className="w-fit rounded-full border border-[#d8c3b2] px-3 py-1 text-xs font-bold text-[#5d4035] transition hover:bg-[#f3e6d8]"
              onClick={onStartBudgetEdit}
              type="button"
      ...
      {isEditingBudget || budgetMessage ? <div className="mt-4 rounded-3xl border border-[#ead9c8] bg-white/60 p-4">
        {isEditingBudget ? (
          <form className="flex flex-col gap-3 sm:flex-row sm:items-end" onSubmit={onBudgetSubmit}>
...
function AddExpenseForm({
  ...
  onCategoryChange: (value: MockExpense['category']) => void;
  ...
  return (
    <>
      ...
      <form className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4" onSubmit={onSubmit}>
        ...
        <label className="flex flex-col gap-2 text-sm font-semibold text-[#5d4035]">
          ...
          <select
            ...
            onChange={(event) => onCategoryChange(event.target.value as MockExpense['category'])}
            value={category}
          >
        ...
        <button
          className="rounded-2xl bg-[#4b2c22] px-5 py-3 text-sm font-bold text-[#fff7ed] shadow-[0_12px_28px_rgba(75,44,34,0.24)] transition hover:bg-[#5b382c] md:col-span-2 xl:col-span-4"
          type="submit"
        >
          Add expense
        </button>
        {formMessage ? <p className="text-sm text-[#7d6253] md:col-span-2 xl:col-span-4">{formMessage}</p> : null}
      </form>
    </>
  );
}
    ...
    <section className="rounded-[1.75rem] border border-[#ead9c8] bg-[#fffaf3] p-5 shadow-[0_18px_48px_rgba(76,45,33,0.10)] sm:p-6">
      ...
      {hasEnoughData ? (
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          ...
          <div className="rounded-2xl border border-[#efe2d4] bg-white/75 p-4 sm:col-span-2">
